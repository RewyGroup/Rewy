    ...
    <Row  onClick={hasCorrectAnswer ? handleCorrectClick : handleOnClick} className="cardRow mr-0 ml-0">
      ...
      <Col className="questionCardInfo" xs={11}>
        ...
        <Row>
          ...
          {hasCorrectAnswer && accordionIsOpen ? (
            <Col xs={6} sm={3}>
              <button
                className="questionCardAccordionButton"
                value={question.id}
          ...
          <Accordion className="questionCardCorrectAnswerAccordion">
            <Card className="questionCardCorrectAnswerCard">
              <Accordion.Toggle
                className="questionCardAccordionToggle"
                id={question.id}
              ...
              <Accordion.Collapse eventKey={question.id}>
                <Card.Body className="questionCardCorrectCardBody">
                  {hasCorrectAnswer && isWYSIWYG ? (
                    <div>
                      <Row className="mr-0 ml-0 questionCardCorrectAnswerInfo">
                        <Col xs={9}>
                          <span className="questionCardCorrectAnswerByText">
                            {" "}
                            Rätt svar av
                          </span>
                          <img
                            className="questionCardThumbnail"
                            src={correctAnswer[0].user.profileImageUrl}
                            alt="thumbnail"
                          />
                          <span className="questionCardCreatedByName">
                            {correctAnswer[0].user.username}
                          </span>
                        </Col>
                        <Col
                          xs={3}
                          className="questionCardCorrectAnswerCreatedAt"
                        >
                          {" "}
                          <span>
                            {correctAnswer[0].createdAt.replace("T", " ")}
                          </span>{" "}
                        </Col>{" "}
                      </Row>{" "}
                      <div className="questionCardCorrectAnswerEditor">
                      <Editor
                     toolbarHidden={true}
